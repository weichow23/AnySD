...
class UNet2DConditionAnySD(
    ...
    def process_encoder_hidden_states(
        ...
        if self.encoder_hid_proj is not None and self.config.encoder_hid_dim_type == "text_proj":
        ...
        elif self.encoder_hid_proj is not None and self.config.encoder_hid_dim_type == "ip_image_proj":  # 选的是这个
            ...
            # TODO: self.encoder_hid_proj可以统一一下？
            image_embeds = self.encoder_hid_proj(added_cond_kwargs["image_embeds"])   # self.encoder_hid_proj is MultiIPAdapterImageProjection
            # list [3, 1, 16, 768] length=1, train: torch.Size([bs, 1, 16, 768]), infer: torch.Size([3, 1, 16, 768])
            # encoder_hidden_states = (encoder_hidden_states, image_embeds)

            # ------ zw@zju: concat task emb here
            # infer:  torch.Size([3, 1, 17, 768])  train: torch.Size([bs, 1, 17, 768])
            task_image_embeds = [torch.cat((added_cond_kwargs["task_embeds"], image_embeds[0]), dim=2)]

            encoder_hidden_states = (encoder_hidden_states, task_image_embeds)
    ...
    def forward(
        ...
        Args:
            ...
            class_labels (`torch.Tensor`, *optional*, defaults to `None`):
                ...
                # zw@zju Emu Edit directly use this
        ...
        )  # encoder_hidden_states -> tuple for input on decoupled attention
