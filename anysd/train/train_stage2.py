# ref to https://github.com/huggingface/diffusers/blob/main/examples/instruct_pix2pix/train_instruct_pix2pix.py
import logging
import math
import wandb
import shutil
...
from anysd.train.valid_log import log_validation_stageii
from anysd.train.dataset import AnyEditMixtureDatasetStageII, collate_fn_anyedit, expert_name_list
from anysd.src.model import IPAdapterPlusTrain, AnySDPipeline
from anysd.src.unet import UNet2DConditionAnySD
from anysd.src.utils import tokenize_captions, choose_book
...
def main():
    ...
    if accelerator.is_main_process:
        if args.report_to == "wandb":
            os.environ["WANDB_API_KEY"] = "WANDB_API_KEY"  # todo: wandb init
    ...
    image_encoder = CLIPVisionModelWithProjection.from_pretrained(args.image_encoder_path, subfolder="image_encoder")
    
    assert args.resume_the_unet is not None, 'please provide the pre-trained unet from stage one'
    unet = UNet2DConditionAnySD.from_pretrained(args.resume_the_unet, subfolder="unet")
    ...
    expert_num, task_book = choose_book(args.expert_name)

    ip_adapter = IPAdapterPlusTrain(unet=unet, image_encoder=image_encoder,
    ...
    optimizer = optimizer_cls(
        ...
        betas=(args.adam_beta1, args.adam_beta2), # todo: 原版没有
        weight_decay=args.adam_weight_decay,
        eps=args.adam_epsilon, # todo: 原版没有
    )
    total_trainable_params = sum(p.numel() for p in optimizer.param_groups[0]['params'] if p.requires_grad)

    # Get the datasets: you can either provide your own training and evaluation files (see below)
    train_dataset = AnyEditMixtureDatasetStageII(args=args, tokenizers=tokenizer,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                #         cprint(f"Parameter {name} did not receive gradient.", 'cyan')
            ...
            if accelerator.is_main_process:
                if (step % (args.checkpointing_steps * args.valid_per_ckpt) == 0) and global_step > 1:
                    task_embs_checkpoints = f"./{args.output_dir}/checkpoint-{(global_step // 100) * 100}/ip_adapter_plus/task_embs.bin"
                    adapter_checkpoints = {
                        "expert-only": task_embs_checkpoints.replace('/task_embs.', '/ip_adapter_sd15.'),
                    }
