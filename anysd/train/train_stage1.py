...
from anysd.train.dataset import AnyEditMixtureDatasetStageI, collate_fn_anyedit
from anysd.src.utils import tokenize_captions
import diffusers
...
from anysd.train.valid_log import log_validation_stagei
...
def main():
    ...
    if args.resume_the_unet is not None:
        unet = UNet2DConditionModel.from_pretrained(args.resume_the_unet)
    else:
        ...
        with torch.no_grad():
            ...
            new_conv_in.weight[:, :4, :, :].copy_(unet.conv_in.weight) # copy from the original one
    ...
    train_dataset = AnyEditMixtureDatasetStageI(args=args, tokenizers=tokenizer, accelerator=accelerator)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0 and global_step > 0:
                    if accelerator.is_main_process:
                        ...
                        # ----- save optimizer at the same time ( Unet size * 5
                        ...
                        #     print('only save ema unet weight')
                        # unwrapped_unet = accelerator.unwrap_model(unet)
                        # # 这里会有问题，存好像只能存成全精度
                        # # if args.variant == 'fp16':
                        # #     unwrapped_unet = unwrapped_unet.half()
                        # unwrapped_unet.save_pretrained(save_path, variant=args.variant)
                        # if args.use_ema:
            ...
            if accelerator.is_main_process:
                if (step % (args.checkpointing_steps * args.valid_per_ckpt) == 0):
                    if args.use_ema:
                        # Store the UNet parameters temporarily and load the EMA parameters to perform inference.
                        ema_unet.store(unet.parameters())
                        ema_unet.copy_to(unet.parameters())
